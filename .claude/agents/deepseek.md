---
name: deepseek
description: "DeepSeek R1, lokales LLM auf NPU (hailo-ollama), Meta-QA, Philosophie, Moloch-Wuerdigkeit, LLM-Integration. Nutze fuer LLM/DeepSeek-Aufgaben."
...
- `core/deepseek_client.py` — OpenAI-kompatibler DeepSeek Chat-Client (Minimal-Impl.)
...
## Hardware-Fakten
- hailo-ollama: Port 8000, Binary installiert, KEIN systemd-Service (noch nicht)
- Modell: deepseek_r1_distill_qwen:1.5b (geladen, ~1.5B Parameter)
- VDevice: SHARED (vdevice-group-id=SHARED) — NIEMALS zweites VDevice (Error 74)
- DeepSeek Cloud API: Key in config/api_keys.json (35 Zeichen), OpenAI-kompatibel
...
- API-Keys NIEMALS committen oder loggen
- timeout=30 fuer alle LLM-HTTP-Calls (NEVER 5)
- DeepSeek URL: /v1/chat/completions (NICHT /chat — das ist falsch!)

## Agent-Lock (PFLICHT)
Erster Schritt vor jeder Datei-Aenderung:
```bash
touch /tmp/moloch_agent_deepseek
```
Letzter Schritt nach abgeschlossener Aufgabe:
```bash
rm /tmp/moloch_agent_deepseek
```
