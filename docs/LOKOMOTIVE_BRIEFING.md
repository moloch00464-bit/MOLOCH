...
## ⛔ PFLICHT-SCHRITT 0 — AGENT LADEN (ABSOLUT VOR ALLEM ANDEREN)
...
**Pi-Reboot PFLICHT** (`sudo reboot`):
- `moloch.service` (systemd Unit) geändert
- `~/.profile` (Umgebungsvariablen wie MOLOCH_USE_TAPPAS) geändert
...
**Nach Reboot (warte ~60 Sekunden, Pi braucht Zeit):**
- Prüfe FPS > 10 (Pipeline läuft)
- Prüfe NPU Worker geladen
- Führe Audit aus
