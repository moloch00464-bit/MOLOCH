...
def collect() -> Dict[str, Any]:
    ...
    try:
        ...
        ok = age_s < ALIVE_MAX_AGE_S or last_ts == 0  # frisch geboot = OK
        if ok:
            score += 1
        checks.append({
            "name": "state_engine_alive",
            "ok": ok,
            "detail": f"last_transition_age_s={age_s:.1f} (max {ALIVE_MAX_AGE_S})",
        })
        detail["current_state"] = snap.get("current_state")
        detail["state_vector"] = snap.get("state_vector")
    ...
    # 2. transition_engine failsafe (kein State stuck > 300s)
    try:
        ...
        checks.append({
            ...
            "ok": ok,
            "detail": f"state_age_s={age_s:.1f} (max {STUCK_FAILSAFE_S})",
        })
    except Exception as e:
        checks.append({
            "name": "transition_engine_failsafe",
